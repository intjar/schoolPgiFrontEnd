...
@Directive({
	...
	selector: '[alphaNumericSearch]',
	providers: [{ provide: NG_VALIDATORS, useExisting: AlphaNumericSearchDirective, multi: true }],
})
export class AlphaNumericSearchDirective implements Validator {
	// eslint-disable-next-line @angular-eslint/no-input-rename
	@Input('alphaNumericSearch') alphaNumericSearch: string = '';
	...
	validate(control: AbstractControl): ValidationErrors | null {
		...
		} else if (this.alphaNumericSearch == 'NR' && control?.value?.length > 0) {
			const isValid = this.regxVal.test(control?.value);
			return !isValid ? { invalidInput: true } : null;
		}
		return null;
